...
const SearchItem = () => {
  ...
  const addToBasket = (e) => {
    ...
    e.target.parentElement.previousElementSibling.firstChild.style.border =
      "1px solid #34495e";
    e.target.parentElement.previousElementSibling.lastChild.style.border =
      "1px solid #34495e";
  ...
  return (
    <div className="search-container">
      <div className="search-representative">
        <p className="after-search" style={{ color: "purple" }}>
          {!isSubmit && `Search your favourite items`}
        </p>
        <p className="after-search" style={{ color: "steelblue" }}>
          {isSubmit &&
            inputValue &&
            filteredData.length > 0 &&
            `Found 1 product with keyword ${inputValue}`}
        </p>
        <p className="after-search" style={{ color: "tomato" }}>
          {isSubmit &&
            filteredData.length <= 0 &&
            `No products found for your keyword ${inputValue}`}
        </p>
      </div>

      <div className="search-items">{isSubmit && searchElements}</div>
    </div>
  );
};
