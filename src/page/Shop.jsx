...
const Shop = () => {
  ...
  const { itemId, setItemId,length, setLength,clearBasket,setClearBasket,status,setStatus,array,setArray,deleteArray,setDeleteArray,filteredData, removeArray, setRemoveArray } 
  ...
  function removePriceAlso(size) {
    const add = array.filter(price => price === size)
    const del = deleteArray.filter(price => price === size)
    const arr = add.slice(del.length)
    setRemoveArray((prev) => {
      return [...prev, ...arr];
    });
  }
