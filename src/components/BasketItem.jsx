...
const BasketItem = ({
  ...
  function removeFromBasket(ID) {
    ...
    if (filteredStatus.length > 0) {
      ...
      filteredStatus[0].parentElement.previousElementSibling.firstChild.style.border =
        "none";
      filteredStatus[0].parentElement.previousElementSibling.lastChild.style.border =
        "none";
    ...
    <>
      <div id={glass.id} className="single-item">
        ...
        <div className="sizes">
          ...
          >
            {menuItem.length > 0 &&
            menuItem.filter((item) => item.id === glass.id).pop()
              ? menuItem.filter((item) => item.id === glass.id).pop().size
              : "28 mm"}
          </p>
        </div>
          ...
          <p
            ...
            style={{
              backgroundColor: `${
                menuItem.length > 0 &&
                menuItem.filter((item) => item.id === glass.id).pop()
                  ? menuItem.filter((item) => item.id === glass.id).pop().color
                  : "red"
              }`,
            }}
          >
        ...
        <p className="amount-per-item">{`$${glass.price * q}.00`}</p>
